...
TARGET_DATASETS=[
     ('train', {'task_name':'onecommon.selection', 'path':'onecommon_data/converted/train.json'}),
     ('valid', {'task_name':'onecommon.selection', 'path':'onecommon_data/converted/valid.json'}),
     ('test_full', {'task_name':'onecommon.selection', 'path':'onecommon_data/converted/test.json'}),
     ('test_success_only', {'task_name':'onecommon.selection', 'path':'onecommon_data/converted/test_success_only.json'}),
]
...
def eval_model(net_module):
    ...
    for key, dataset_spec in TARGET_DATASETS:
        dataset_spec['provide_image'] = config.provide_image
        dataset = VisualSelectionDataset(
            dataset_list=[dataset_spec],
            textifier=textifier,
            provide_image=config.provide_image,
            image_shape=config.model_net_args.get('image_shape', (1, 1)),
    ...
    model_net = net_module.SelectorNet(textifier.get_len(), **config.model_net_args)
